...
resource "opentelekomcloud_dns_recordset_v2" "mysqlrds_nor1tst_com" {
  zone_id = "${var.zone_id}"
  name = "${opentelekomcloud_rds_instance_v1.instance_mysqlrds.name}.otc.nortst.com."
  description = "nor1 A-Name record set"
  ttl = 3000
  type = "A"
  records = ["${opentelekomcloud_rds_instance_v1.instance_mysqlrds.hostname}"]
  depends_on = ["opentelekomcloud_rds_instance_v1.instance_mysqlrds"]
}
