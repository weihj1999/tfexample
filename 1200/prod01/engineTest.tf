...
resource "opentelekomcloud_dns_recordset_v2" "otc01_nortst_com" {
  count = "${var.engine_count}"
  zone_id = "${var.zone_id}"
  name = "${element(opentelekomcloud_compute_instance_v2.az01-grid-engine.*.name, count.index)}.otc.nortst.com."
  description = "nor A-Name record set"
  ttl = 3000
  type = "A"
  records = ["${element(opentelekomcloud_compute_instance_v2.az01-grid-engine.*.access_ip_v4, count.index)}"]
}
