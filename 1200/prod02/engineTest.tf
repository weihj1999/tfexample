...
resource "opentelekomcloud_blockstorage_volume_v2" "az02-grid-engine-sys-vol" {
...
resource "opentelekomcloud_blockstorage_volume_v2" "az02-grid-engine-data-vol" {
   count = "${var.engine_count}"
...
resource "opentelekomcloud_compute_instance_v2" "az02-grid-engine" {
   count           = "${var.engine_count}"
   name            = "winaz02-engine${count.index}"
   ...
   block_device {
     uuid = "${element(opentelekomcloud_blockstorage_volume_v2.az02-grid-engine-sys-vol.*.id,count.index)}"
     source_type           = "volume"
     boot_index            = 0
     destination_type      = "volume"
     delete_on_termination = false
   }
   depends_on = ["opentelekomcloud_blockstorage_volume_v2.az02-grid-engine-sys-vol"]
...
resource "opentelekomcloud_compute_volume_attach_v2" "engine_volume_attach" {
   count       = "${var.engine_count}"
   instance_id = "${element(opentelekomcloud_compute_instance_v2.az02-grid-engine.*.id, count.index)}"
   volume_id   = "${element(opentelekomcloud_blockstorage_volume_v2.az02-grid-engine-data-vol.*.id, count.index)}"
   depends_on = ["opentelekomcloud_compute_instance_v2.az02-grid-engine","opentelekomcloud_blockstorage_volume_v2.az02-grid-engine-data-vol"]
}

resource "opentelekomcloud_dns_recordset_v2" "otc02_nortst_com" {
  count = "${var.engine_count}"
  zone_id = "${var.zone_id}"
  name = "${element(opentelekomcloud_compute_instance_v2.az02-grid-engine.*.name, count.index)}.otc.nortst.com."
  description = "nor A-Name record set"
  ttl = 3000
  type = "A"
  records = ["${element(opentelekomcloud_compute_instance_v2.az02-grid-engine.*.access_ip_v4, count.index)}"]
}
