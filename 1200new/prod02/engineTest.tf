...
resource "dns_a_record_set" "otc_prod02_nortst_com" {
  count = "${var.engine_count}"
  zone = "otc.nortst.com."
  name = "${element(opentelekomcloud_compute_instance_v2.az02-grid-engine.*.name, count.index)}"
  addresses = [
    "${element(opentelekomcloud_compute_instance_v2.az02-grid-engine.*.access_ip_v4, count.index)}"
  ]
  ttl = 3000
}
